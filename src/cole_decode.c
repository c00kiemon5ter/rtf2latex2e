...
/* verbose pps tree */
void
verbosePPSTree (pps_entry * pps_list, U32 start_entry, int level)
{
  U32 entry;
  int i;
  ...
  for (entry = start_entry; entry != 0xffffffffUL; entry = pps_list[entry].next)
      ...
      if (pps_list[entry].type == 2)
	{
	  ...
	  printf ("FILE %02x %8d '%c%s'\n", pps_list[entry].ppsnumber,
		  pps_list[entry].size, !isprint(pps_list[entry].name[0]) ? ' ' : pps_list[entry].name[0], pps_list[entry].name+1);
	}
      else
	{
	  for (i = 0; i < level * 3; i++)
	    printf (" ");
	  printf ("DIR  %02x '%c%s'\n", pps_list[entry].ppsnumber,
		  !isprint(pps_list[entry].name[0]) ? ' ' : pps_list[entry].name[0], pps_list[entry].name+1);
