...
#define DOTS_PER_CM     2       /* JFIF unit byte: dots per cm       */
    ...
    if (image.height <= 0 || image.width <= 0 || image.components <= 0) {
        fprintf(stderr,
                "Error: DNL marker not supported in PostScript Level 2!\n");
    }
...
void
WriteEPSHeader(imagedata * JPEG, FILE * PSfile, int *llx1, int *lly1,
               int *urx1, int *ury1)
{
    ...
    if (JPEG->adobe && JPEG->components == 4) {
        if (!quiet)
            fprintf(stderr,
                    "Note: Adobe-conforming CMYK file - applying workaround for color inversion.\n");
